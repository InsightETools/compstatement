...
window.reloadFromParams = async () => {
  ...
  try {
    // ⛔️ Prevent stale JSON on employee change
    const res = await fetch(fetchUrl, { signal: currentFetchController.signal, cache: "no-store" });
...
async function renderAll(data) {
  ...
  function standardTables() {
    ...
    // 🔍 Hide/clear any standard* containers that aren’t in this payload
    const validStandardIds = new Set((data.standardTables || []).map(t => `standard${t.id}`));
    document.querySelectorAll('[id^="standard"]').forEach(el => {
      if (!validStandardIds.has(el.id)) {
        el.innerHTML = "";
        el.style.display = "none";
      } else {
        el.style.display = "";
  ...
  function booleanTables() {
    ...
    // 🔍 Hide/clear boolean containers not present in this payload (supports multiple conventions)
    const validBoolIds = new Set((data.booleanTables || []).map(t => String(t.id)));
    const possibleContainers = document.querySelectorAll(
      '.booleantablecontainer[id], [booleantable][id], [id^="boolean"], [data-type="booleantable"]'
    );
    possibleContainers.forEach(el => {
      if (!validBoolIds.has(el.id)) {
        el.innerHTML = "";
        el.style.display = "none";
      } else {
        el.style.display = "";
