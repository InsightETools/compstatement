// ===== Build v25.001.009 (Namespaced; conflict-safe) =================
(function EncodedUIBootstrap(global) {
  "use strict";

  // Create a single namespace for this bundle
  const EncUI = global.EncUI || (global.EncUI = {});

  /* ------------------ DOM helpers (local only) ------------------ */
  const $  = (sel) => document.querySelector(sel);
  const $$ = (sel) => document.querySelectorAll(sel);

  /* ------------------ URL param helpers (local, encoded) ------------------ */
  // Uses '+' for spaces; safe within this module only.
  ...
  /* ------------------ State (kept private to this module) ------------------ */
  let isLoaded = false;
  let currentFetchController = null;
  EncUI.currentData = null; // exported state if needed externally
  ...
  function buildFetchUrlFromParams() {
    ...
    if (!key) {
      // local JSON; encode only ek part
      return `https://compstatementdemo.netlify.app/data/${_enc(ek)}.json`;
    }

    // Server accepts standard encoding
    const qp = new URLSearchParams({ usecors: "1", key, cpid, yr, ck, ek, layout });
    return `${baseUrl}?${qp.toString()}`;
  }

  const debouncedReloadFromParams = debounced(() => EncUI.reloadFromParams(), 60);

  /* ------------------ Overflow fit (local only) ------------------ */
  ...
  async function renderAll(data) {
    ...
    function donutCharts() {
      ...
      wrappers.forEach((wrapper, wIdx) => {
        ...
        charts.forEach((chart, cIdx) => {
          ...
          const theDiscEl  = clone.querySelector('[category="disclaimer"]');
    ...
    EncUI.currentData = data;
    renderPrice(EncUI.currentData);
  ...
  /* ------------------ Public reload (namespaced) ------------------ */
  EncUI.reloadFromParams = async () => {
    ...
    try {
      ...
      // use local applyOverflow (do not touch window.applyOverflow)
      applyOverflow();
  ...
  (function controls() {
    ...
    const applyLayout = (val, { reload = true } = {}) => {
      ...
      applyOverflow();
      try { /* re-render charts if needed */ } catch {}
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (exploreToggle) {
        exploreToggle.addEventListener("change", (e) => {
          if (e.target.checked) setMode("explore"); // sets ?mode=explore
        });
      }
      if (pricingToggle) {
        pricingToggle.addEventListener("change", (e) => {
          if (e.target.checked) setMode("pricing"); // sets ?mode=pricing
        });
      }
  ...
  // ------------------ Public API ------------------
  EncUI.setMode = setMode;
  EncUI.getParams = getParams;
  EncUI.setParam = setParam;

  console.log("Build v25.002.000 (EncUI namespace)");
})(window);
// ===================================================================
