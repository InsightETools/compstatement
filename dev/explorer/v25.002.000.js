// ===== Explorer Build v25.002.000 (Namespaced; conflict-safe) =================
(function ExplorerUIBootstrap(global) {
  "use strict";

  // Create namespace for Explorer
  const ExpUI = global.ExpUI || (global.ExpUI = {});
  
  // Check if shared utilities exist, otherwise create them
  const SharedUI = global.SharedUI || (global.SharedUI = {});

  /* ------------------ Shared Utilities (only create if not exists) ------------------ */
  if (!SharedUI.hexToRgb) {
    SharedUI.hexToRgb = function(hex) {
      hex = hex.replace(/^#/, "");
  ...
  if (!SharedUI.formatCurrency) {
    SharedUI.formatCurrency = function(value, element = null, decimalFlag = null, isCurrency = true) {
  ...
  if (!SharedUI.debounced) {
    SharedUI.debounced = function(fn, ms = 60) {
      let t;
      return (...args) => {
        clearTimeout(t);
  ...
  if (!SharedUI.getParams) {
    SharedUI.getParams = function() {
      return new URLSearchParams(window.location.search);
    };
  }

  if (!SharedUI.setParam) {
    SharedUI.setParam = function(key, value) {
      const p = SharedUI.getParams();
  ...
  function _collectButtons() {
    if (ExpUI._buttonsCollected) return; // Prevent duplicate collection
  ...
  function computeDesignConstraintsAndApply() {
    ...
    const params = SharedUI.getParams();
  ...
  global.applyOverflow = ExpUI.applyOverflow;
  ...
  function renderPrice(data) {
    ...
    document.querySelectorAll('[details="price"]').forEach((el) => {
      el.textContent = SharedUI.formatCurrency(total, el, true, true);
    });
  }
  ...
  function renderDonutChart({ chartId, categoryGroup, containerSelector }) {
    ...
    categoryGroup.forEach((category) => {
      ...
      const color = SharedUI.hexToRgb(category.color);
  ...
  async function renderAll(data) {
          ...
          table.categories.forEach((category) => {
            ...
            category.items.forEach((lineitem, index) => {
              ...
              if (showCol1) {
                ...
                col1Div.textContent = SharedUI.formatCurrency(lineitem.col1_value, col1Div, table.isDecimal);
              ...
              if (showCol2) {
                ...
                col2Div.textContent = SharedUI.formatCurrency(lineitem.col2_value, col2Div, table.isDecimal);
              ...
              if (showCol3) {
                ...
                col3Div.textContent = SharedUI.formatCurrency(lineitem.col3_value, col3Div, table.isDecimal);
            ...
            if (showCol1) {
              ...
              subCol1.textContent = SharedUI.formatCurrency(category.col1_subtotal, subCol1, table.isDecimal);
            ...
            if (showCol2) {
              ...
              subCol2.textContent = SharedUI.formatCurrency(category.col2_subtotal, subCol2, table.isDecimal);
            ...
            if (showCol3) {
              ...
              subCol3.textContent = SharedUI.formatCurrency(category.col3_subtotal, subCol3, table.isDecimal);
      ...
      (data.booleanTables || []).forEach((tableData) => {
        ...
        for (const [labelKey, columnAttr] of Object.entries(columnMap)) {
          ...
          if (isMissing) {
            ...
            if (totalEl) {
              ...
              totalEl.textContent = SharedUI.formatCurrency(tableData[totalKey], totalEl, tableData.isDecimal);
            }
          }
        }
        ...
        tableData.categories.forEach((itemData, index) => {
          ...
          Object.entries(itemData).forEach(([key, value]) => {
            ...
            if (key === "colBool_value" && cell) {
              ...
              cell.textContent = SharedUI.formatCurrency(value, cell, tableData.isDecimal);
            } else if (cell) {
        ...
        container
          ...
          .forEach((el) => {
            ...
            el.textContent = SharedUI.formatCurrency(parseFloat(value), el, tableData.isDecimal);
          });
      });
    }
          ...
          (module.components || []).forEach((component) => {
            ...
            if (valueEl) {
              ...
              const formattedValue = needsFormatting
                ? SharedUI.formatCurrency(value, valueEl, module.isDecimal, isCurrency)
                : value;
    ...
    function donutCharts() {
      ...
      wrappers.forEach((wrapper, wIdx) => {
        ...
        charts.forEach((chart, cIdx) => {
          ...
          if (totalEl) totalEl.textContent = SharedUI.formatCurrency(chart.totalValue, totalEl, chart.isDecimal);
          ...
          if (indexWrapper) {
            ...
            (chart.groups || []).forEach((group) => {
              ...
              colorBox.style.backgroundColor = SharedUI.hexToRgb(group.color);
    ...
    function loadDisplay() {
      const renderParam = SharedUI.getParams().get("render");
      if (renderParam === "true") {
    ...
    ExpUI.currentData = data;
    renderPrice(ExpUI.currentData);
  ...
  /* ------------------ Public reload ------------------ */
  const debouncedReloadFromParams = SharedUI.debounced(() => ExpUI.reloadFromParams(), 60);

  ExpUI.reloadFromParams = async () => {
  ...
  global.reloadFromParams = ExpUI.reloadFromParams;
  global.__currentData = ExpUI.currentData;
  ...
  (function controls() {
    ...
    const safeBind = (el, handler) => {
      if (!el) return;
      if (el.dataset.expUiBound) return; // Prevent duplicate binding
      el.dataset.expUiBound = "true";
    ...
    const getCurrentDesign = () => SharedUI.getParams().get("design") || "1";
    const getCurrentLayout = () => SharedUI.getParams().get("layout") || "1";
    const getCurrentHeader = () => SharedUI.getParams().get("header") || "1";
    const getCurrentCover  = () => SharedUI.getParams().get("cover") ?? "0";
    ...
    const applyLayout = (val, { reload = true } = {}) => {
      ...
      SharedUI.setParam("layout", val);

      computeDesignConstraintsAndApply();
      _applyEffectiveButtonStates();

      if (typeof ExpUI.applyOverflow === "function") ExpUI.applyOverflow();
      try { /* donutCharts if needed */ } catch {}
    ...
    const applyDesignSwitch = (val, { reload = true } = {}) => {
      ...
      if (val === "2") SharedUI.setParam("cover", "false");

      SharedUI.setParam("design", val);
      updateExtras();
      ...
      if (val === "2") {
        ...
        if (!SharedUI.getParams().has("layout")) SharedUI.setParam("layout", "1");
    ...
    const updateExtras = () => {
      ...
      ["benefits", "company"].forEach((key) => {
        const enabled = SharedUI.getParams().get(key) === "true";
      ...
      ["0", "1", "2", "3"].forEach((k) => toggleActive("cover" + k, showCover && coverParam === k));
    ...
    const toggleExtra = (key) => {
      const current = SharedUI.getParams().get(key) === "true";
      SharedUI.setParam(key, (!current).toString());
      updateExtras();
      computeDesignConstraintsAndApply();
      _applyEffectiveButtonStates();
    ...
    function selectEmployee(ekId) {
      SharedUI.setParam("ek", ekId);
      window.location.reload();
    }

    document.addEventListener("DOMContentLoaded", () => {
      if (ExpUI._controlsInitialized) return; // Prevent duplicate initialization
      ExpUI._controlsInitialized = true;
      ...
      if (designBtns.length) {
        designBtns.forEach((btn) => {
          btn.addEventListener("click", () => {
            ...
            SharedUI.setParam("design", idNum);
      ...
      if (demoEl) demoEl.style.display = SharedUI.getParams().get("demo") === "true" ? "" : "none";
      ...
      if (!SharedUI.getParams().has("layout")) SharedUI.setParam("layout", "1");
      if (!SharedUI.getParams().has("header")) SharedUI.setParam("header", "1");
      if (!SharedUI.getParams().has("cover"))  SharedUI.setParam("cover", "0");
      if (!SharedUI.getParams().has("ek"))     SharedUI.setParam("ek", "EmployeeA");
  ...
  console.log("Explorer Build v25.002.000 (ExpUI namespace)");
})(window);
